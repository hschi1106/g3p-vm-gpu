...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Download PSB2 datasets into the repository data layout. "
            "By default, downloads all PSB2 problems."
        )
    )
    parser.add_argument(
        "--out-dir",
        default="data/psb2_datasets",
        help="Target datasets directory (default: data/psb2_datasets).",
    )
    parser.add_argument(
        "--problems",
        default="all",
        help="Comma-separated PSB2 problem names, or 'all' (default: all).",
    )
    parser.add_argument(
        "--splits",
        default="edge,random",
...
def _resolve_problems(raw: str) -> List[str]:
    if raw.strip().lower() == "all":
        return list(PSB2_PROBLEMS)
    picked = _split_csv(raw)
    unknown = [p for p in picked if p not in PSB2_PROBLEMS]
    if unknown:
        raise ValueError(
            "unknown PSB2 problem(s): "
            + ", ".join(unknown)
...
def _download_with_retries(url: str, dest: Path, timeout_sec: int, retries: int) -> None:
...
def main() -> int:
    ...
    try:
        ...
        print(f"[psb2] argument error: {exc}", file=sys.stderr)
        return 2
    ...
    for problem, split, dest, url in _iter_targets(out_dir, problems, splits):
        ...
        if dest.exists() and dest.stat().st_size > 0 and not args.force:
            skipped += 1
            print(f"[psb2] skip  {problem:20s} {split:6s} {dest}")
            continue

        print(f"[psb2] fetch  {problem:20s} {split:6s} {url}")
        if args.dry_run:
            continue
        try:
            _download_with_retries(url=url, dest=dest, timeout_sec=args.timeout_sec, retries=args.retries)
            downloaded += 1
        except Exception as exc:  # keep going to report all failures
            failed += 1
            print(f"[psb2] fail  {problem:20s} {split:6s} {exc}", file=sys.stderr)

    print(
        f"[psb2] done total={total} downloaded={downloaded} skipped={skipped} failed={failed} "
        f"out_dir={out_dir}"
    )
