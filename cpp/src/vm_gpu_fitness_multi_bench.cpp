...
int main(int argc, char** argv) {
  ...
  std::vector<int> fitness =
      g3pvm::run_bytecode_gpu_multi_fitness_shared_cases(programs, shared_cases, shared_answer, fuel,
                                                         blocksize);
  ...
  std::cout << "OK programs=" << program_count << " cases_per_program=" << cases_per_program
            << " fuel=" << fuel << " blocksize=" << blocksize << " elapsed_ms=" << ms
            << " pass_fit=" << ok_programs << " neg_fit=" << neg_programs << "\n";
  return 0;
}
